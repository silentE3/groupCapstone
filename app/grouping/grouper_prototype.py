...
class Grouper:
    '''
    This class is used to create groups via constructive, heuristic approach with local backtracking.
    '''
    ...
    def create_groups(self) -> list[models.GroupRecord]:
        ...
        '''
        # "Step 5": Repeat Steps 1-4 "grouping_passes" number of times (specified in the config)
        for outer_loop_count in range(max(self.config_data["grouping_passes"], 1)):
            self.groups = []
            ...
            non_stand_mod: int = 0
            if (self.num_groups * self.config_data["target_group_size"]) < len(self.survey_data):
                non_stand_mod = 1
            elif (self.num_groups * self.config_data["target_group_size"]) > len(self.survey_data):
                non_stand_mod = -1
            ...
            if non_stand_mod == 1:
                num_non_targ_groups = len(
                    self.survey_data) % self.config_data["target_group_size"]
            if non_stand_mod == -1:
                num_non_targ_groups = self.config_data["target_group_size"] - \
                    (len(self.survey_data) %
                ...
                for group in self.groups:
                    ...
                    if ((len(group.members) > self.config_data["target_group_size"]) or
                            ...
                            (non_stand_mod == -1 and len(group.members) == (self.config_data["target_group_size"] - 1) and
                             count_groups_max_size >= (self.num_groups - num_non_targ_groups))):
                        continue
            ...
            self.scoring_vars = models.GroupSetData("solution_1",
                                                    ...
                                                    sum(len(group.members) for group in self.groups))
            ...
            while ((validate.total_disliked_pairings(self.groups) > 0) and
                ...
                loop_count += 1
                print("Loop " + str(outer_loop_count + 1) +
            ...
            if (self.best_solution_score < 0 or (self.cur_sol_score < self.best_solution_score) or
        ...
        loop_count = 0
        student_swapped = True
        while (loop_count < max((self.config_data["grouping_passes"]*10), 100) and self.cur_sol_score > 0 and student_swapped):
            loop_count += 1
        ...
        for group in self.groups:
            ...
            for idx, student in enumerate(group.members):
                ...
                self.scoring_vars.num_groups_no_overlap = validate.total_groups_no_availability(
                    self.groups)
                self.scoring_vars.num_of_disliked_pairs = validate.total_disliked_pairings(
                    self.groups)
                self.scoring_vars.num_of_preferred_pairs = validate.total_liked_pairings(
                    self.groups)
