...
read-report REPORTFILE

The read-report command reads in a given previously generated report. Ensure that the **reportfile** is the excel file generated by the group or report command. We should be able to read
into a report based on a given report excel file.

1. **reportfile**: the first required parameter for the read-report command. This should be a valid report file in excel generated by the group or report command. 
Its default value is group_report.xlsx.

update-report REPORTFILE

The update-report command properly updates a report based on the given **reportfile** and outputs the same report with new result info based on the manual group changes
made to it. Ensure that the **reportfile** is the excel file generated from when you run the group or report command.
